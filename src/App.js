...
class App extends Component {
  ...
  render() {
    ...
    const lengthOfsearchResult = searchResults.length === 0
    return (
      <>
        ...
        <div className="historyContentContainer">
          <ul className="historyItemList">
            {lengthOfsearchResult ? (
              <EmptyView text="There is no history to show" />
            ) : (
              searchResults.map(eachItem => (
                <HistoryItem
                  key={eachItem.id}
                  historyListItems={eachItem}
                  deleted={this.delete}
                />
              ))
            )}
